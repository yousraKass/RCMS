...
//SALES AND COSTS TREATED IN THEIR CLASSES 
    //SALES AND COSTS TREATED IN THEIR CLASSES  
    Restaurant(Type T, string Name, int id,const Date &creation,const int& employee) :info(T), ID(id), creationDate(creation),employee_num(employee) {  
        leftChild=nullptr;  
        rightChild=nullptr;  
    }  
   ...
   /* void showRestaurantData() const 
    ...
    void reportOnsales(Date date){ 
        SalesAndCosts.reportOnsales(month,year); 
    } 
    void reportOnsales(Date date1,Date date2){ 
         SalesAndCosts.reportOnsales(int day1, int month1, int year1, int day2, int month2, int year2); 
    }*/ 
}; 
#endif